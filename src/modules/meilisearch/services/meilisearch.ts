...
function isProductIndexSettings(config: ProductIndexSettings | CategoryIndexSettings): config is ProductIndexSettings {
  return config.type === SearchUtils.indexTypes.PRODUCTS
}

function isCategoryIndexSettings(
  config: ProductIndexSettings | CategoryIndexSettings,
): config is CategoryIndexSettings {
  return config.type === SearchUtils.indexTypes.PRODUCT_CATEGORIES
}
...
export class MeiliSearchService extends SearchUtils.AbstractSearchService {
  ...
  private async getTransformedDocuments(indexKey: string, documents: any[], options?: TransformOptions) {
    ...
    if (isProductIndexSettings(indexConfig)) {
      return Promise.all(
        documents.map(
          (doc) =>
            indexConfig.transformer?.(doc, transformProduct, { ...options }) ??
            transformProduct(doc, options),
        ),
      )
    }

    if (isCategoryIndexSettings(indexConfig)) {
      return Promise.all(
        documents.map(
          (doc) =>
            indexConfig.transformer?.(doc, transformCategory, { ...options }) ??
            transformCategory(doc, options),
        ),
      )
    }
